 ...
 * the Version: header and BLT_POPUPS_VERSION in blt-popups.php, and the
 * Stable tag in readme.txt. Exits non-zero unless every replacement
 * matched exactly once. Prints the new version to stdout.
 *
 * The CLI block below is guarded so a test harness can include this file
 * for blt_popups_next_version() without side effects.
 *
 * @package Blt_Popups
 */
 ...
 */
function blt_popups_next_version( $current, $level ) {
 ...
 *
 * @param string $plugin_file Contents of blt-popups.php.
 ...
 */
function blt_popups_apply_bump( $plugin_file, $readme, $level ) {
	if ( ! preg_match( '/^ \* Version:\s+(\d+\.\d+\.\d+)$/m', $plugin_file, $m ) ) {
		throw new RuntimeException( 'Could not find the Version: header in blt-popups.php' );
	}
	$next = blt_popups_next_version( $m[1], $level );

	$replacements = array(
		'plugin_file' => array(
			array( '/^( \* Version:\s+)\d+\.\d+\.\d+$/m', '${1}' . $next ),
			array( "/(define\( 'BLT_POPUPS_VERSION', ')\d+\.\d+\.\d+(' \);)/", '${1}' . $next . '${2}' ),
		),
		'readme'      => array(
...
if ( PHP_SAPI === 'cli' && isset( $argv[0] ) && realpath( $argv[0] ) === __FILE__ ) {
	...
	try {
		$result = blt_popups_apply_bump(
			(string) file_get_contents( $root . '/blt-popups.php' ),
	...
	file_put_contents( $root . '/blt-popups.php', $result['plugin_file'] );
